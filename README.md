...
`paper-zh-layout-reader` is a Codex skill for converting English academic paper PDFs into Chinese-only HTML/PDF files that preserve the original paper layout as closely as practical.

It is designed around one fixed output preference: `paper_zh_final_layout.html/pdf` is the final reading paper. Bilingual, linear, source-map, and audit artifacts exist only to check omissions and record limitations.
...
- `paper.md` — source-grounded original/Chinese audit record, not the final reading paper
...
If a side-by-side bilingual file is needed, generate it only as an audit artifact such as `paper_full_bilingual.html/pdf`. If a hard PDF forces a linear fallback such as `paper_zh_full.html/pdf`, mark it as fallback/draft only; it is not a substitute for `paper_zh_final_layout.html/pdf`.
...
The validation step defaults to `paper_zh_final_layout.html/pdf` and checks links, JSON, MathML counts, PDF previews, and whether a `coverage_audit.md` review exists. The scripts expect Python packages such as `pypdf`, and optionally `pypdfium2` for page rendering and PDF preview validation.
...
- keep any bilingual or linear comparison output as audit-only/fallback-only, and provide `paper_zh_final_layout.html/pdf` as the Chinese-only original-layout final reading version.
- if only a linear output can be generated, mark the original-layout output as missing or low-confidence in `coverage_audit.md` and `translation_notes.md`; do not present the linear version as the final same-layout paper.

## Notes On Equations

HTML/PDF output cannot contain native MathType OLE objects. This skill uses MathML as the interoperable equation source:

- MathML is embedded in `paper_zh_final_layout.html`.
